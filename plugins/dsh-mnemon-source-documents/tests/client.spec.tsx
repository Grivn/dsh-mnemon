...
describe('independent Documents Source client', () => {
  it('orders active, archived and searched documents by creation before pagination, including after edits', async () => {
    ...
    const list = within(await screen.findByRole('complementary', { name: t('documents.list') }))
    const titles = () => list.getAllByRole('button').flatMap(item => item.querySelector('strong')?.textContent ?? [])
    await waitFor(() => expect(titles()).toEqual([10, 9, 8, 7, 6, 5, 4, 3].map(day => `Document ${day}`)))
    expect(await screen.findByRole('heading', { name: 'Document 10' })).not.toBeNull()
    expect(screen.queryByRole('button', { name: t('documents.edit') })).toBeNull()
    fireEvent.click(screen.getByRole('button', { name: t('common.showMore', { count: 2 }) }))
    expect(titles().slice(-2)).toEqual(['Document 2', 'Document 1'])
    expect(list.getByRole('button', { name: /Document 1 / }).querySelector('time')?.dateTime).toBe('2026-08-01T08:00:00.000Z')
    fireEvent.change(screen.getByRole('textbox', { name: t('documents.searchAria') }), { target: { value: 'Searchable evidence' } })
    fireEvent.click(screen.getByRole('button', { name: t('documents.search') }))
    await waitFor(() => expect(screen.getByRole('button', { name: t('documents.refresh') }).hasAttribute('disabled')).toBe(false))
    expect(titles()).toEqual([10, 9, 8, 7, 6, 5, 4, 3].map(day => `Document ${day}`))
    fireEvent.click(screen.getByRole('button', { name: new RegExp(t('documents.archivedCount')) }))
    await waitFor(() => expect(titles()).toEqual(['Document 12', 'Document 11']))
    fireEvent.change(screen.getByRole('textbox', { name: t('documents.searchAria') }), { target: { value: '' } })
    fireEvent.click(screen.getByRole('button', { name: t('documents.search') }))
    await waitFor(() => expect(screen.getByRole('button', { name: t('documents.refresh') }).hasAttribute('disabled')).toBe(false))
    expect(titles()).toEqual(['Document 12', 'Document 11'])
    view.rerender(page(true))
    fireEvent.click(screen.getByRole('button', { name: new RegExp(t('documents.active') + ' 10') }))
    expect(await screen.findByRole('button', { name: t('documents.edit') })).not.toBeNull()
